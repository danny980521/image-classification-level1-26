...
def train(data_dir, model_dir, args):
    ...
    for epoch in range(args.epochs):
        ...
        for idx, train_batch in enumerate(train_loader):
            ...
            age_loss = criterion(outs, labels)
            
            
            outs_normal = (outs - torch.min(outs)) / (torch.max(outs) - torch.min(outs))
            ...
            original_criterion = nn.CrossEntropyLoss(weight = weights)
            original_loss = original_criterion(final_outs ,org_labels)
        ...
        with torch.no_grad():
            ...
            print(
                ...
                f"best acc : {best_val_acc:4.2%}, best loss: {best_val_loss:4.2}, best f1 score: {best_val_f1}"
            )
