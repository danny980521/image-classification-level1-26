...
# 마스크 데이터셋을 통해 CNN 모델 학습을 진행하고 완성된 모델을 저장하는 파일
# 다양한 hyperparameter 들과 커스텀 model, data augmentation 등을 통해 성능 형성울 이룰 수 있음
...
def grid_image(np_images, gts, preds, n=10, shuffle=False):
...
def train(data_dir, model_dir, args):
    ...
    wandb.init(project='maskdetection', entity='pkpete', name=Path(save_dir).stem)
    ...
    if args.optimizer == 'AdamP':
        optimizer = AdamP(model.parameters(), lr = args.lr, betas=(0.9,0.99), weight_decay=5e-4)
    else:
    ...
    for epoch in range(args.epochs):
        ...
        if input_figure is None:
            ...
            input_figure = grid_image(
                    train_np, labels, preds, n=10, shuffle=args.dataset != "MaskSplitByProfileDataset"
            )
        ...
        with torch.no_grad():
            ...
            for val_batch in val_loader:
                ...
                if figure is None:
                    ...
                    figure = grid_image(
                        inputs_np, labels, preds, n=11, shuffle=args.dataset != "MaskSplitByProfileDataset"
                    )
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_dir', type=str, default=os.environ.get('SM_CHANNEL_TRAIN', '/opt/ml/input/data/train/new_images'))
