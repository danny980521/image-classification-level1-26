...
class AlbumentationAugmentation:
    def __init__(self, resize, mean, std, **args):
        self.transform = albumentations.Compose([
            # albumentations.CenterCrop(350, 300),
...
class MaskSplitByProfilenLabelDataset(MaskBaseDataset):
    ...
    def setup(self):
        ...
        for profile in profiles:
            ...
            for file_name in os.listdir(img_folder):
                ...
                self.gender_labels.append(gender_label)
                self.age_labels.append(age_label)
                self.all_labels.append(self.encode_multi_class(mask_label, gender_label, age_label)) # �~T�~@
...
class AgeModelDataset(MaskBaseDataset):
    ...
    def setup(self):
        ...
        for profile in profiles:
            ...
            for file_name in os.listdir(img_folder):
                ...
                self.groups.append(id)
                cnt += 1
        pseudo_csv = pd.read_csv('/opt/ml/input/data/eval/pseudo_label.csv')
        for pseudo_img_path in pseudo_csv['img_path']:
            self.image_paths.append(pseudo_img_path)
            self.indexs.append(cnt)
            self.groups.append(os.path.basename(pseudo_img_path))
            cnt += 1
        for pseudo_label in pseudo_csv['class']:
            self.age_labels.append(pseudo_label)
