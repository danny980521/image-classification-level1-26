...
def train(data_dir, model_dir, args):
    ...
    for epoch in range(args.epochs):
        ...
        for idx, train_batch in enumerate(train_loader):
            ...
            _, _, age_labels = MaskBaseDataset.decode_multi_class(labels)
            ...
            loss = criterion(outs, age_labels)
            ...
            f1 += f1_score(age_labels.cpu().numpy(), preds.cpu().numpy(), average='macro').item()
            ...
            if (idx + 1) % args.log_interval == 0:
                ...
                print(
                    ...
                    f"training f1 {f1:3.2%} || lr {current_lr}"
                )
        ...
        with torch.no_grad():
            ...
            for val_batch in val_loader:
                ...
                loss_item = criterion(outs, age_labels).item()
                acc_item = (age_labels == preds).sum().item()
                ...
                score += f1_score(age_labels.cpu().numpy(), preds.cpu().numpy(), average = 'macro')
            score = score / len(val_loader)
            # print(f"Validation F1 Score : {score:3.2%}")
            ...
            if score > best_val_f1:
                ...
                torch.save(model.module.state_dict(), f"{save_dir}/best_{score:.3}.pth")
                best_val_f1 = score
                best_epoch = epoch
            torch.save(model.module.state_dict(), f"{save_dir}/last.pth")
            print(
                f"[Val] f1: {score:3.2%}, acc : {val_acc:4.2%}, loss: {val_loss:4.2} || "
                f"best epoch : {best_epoch}, best acc : {best_val_acc:4.2%}, best loss: {best_val_loss:4.2}, best f1 score: {best_val_f1:3.2%}"
            )
...
if __name__ == '__main__':
    ...
    parser.add_argument('--wce', default=False, action='store_true', help='use weighted cross entropy')
